...
GO
PRINT N'Index "[config].[Message].[IDX_Message_Name_Inc_Content]" wird gelöscht...';
...
GO
PRINT N'Tabelle "[config].[Message]" wird geändert...';


GO
ALTER TABLE [config].[Message] ALTER COLUMN [Name] VARCHAR (128) NOT NULL;


GO
PRINT N'Index "[config].[Message].[IDX_Message_Name_Inc_Content]" wird erstellt...';
...
GO
PRINT N'Tabelle "[config].[DesiredTimeZone]" wird erstellt...';
...
GO
PRINT N'Tabelle "[config].[DiscordMapping]" wird erstellt...';
...
GO
PRINT N'Tabelle "[config].[PersonalReminder]" wird erstellt...';


GO
CREATE TABLE [config].[PersonalReminder] (
    [PersonalReminderID] INT            IDENTITY (1, 1) NOT NULL,
    [CronSchedule]       VARCHAR (64)   NOT NULL,
    [DiscordChannelID]   DECIMAL (20)   NOT NULL,
    [UserToRemind]       DECIMAL (20)   NOT NULL,
...
GO
PRINT N'Tabelle "[config].[SpamProtectedChannel]" wird erstellt...';


GO
CREATE TABLE [config].[SpamProtectedChannel] (
    [SpamProtectedChannelID] DECIMAL (20) NOT NULL,
...
GO
PRINT N'Tabelle "[config].[UnitsEndpoint]" wird erstellt...';


GO
CREATE TABLE [config].[UnitsEndpoint] (
    [UnitsEndpointID]           INT           IDENTITY (1, 1) NOT NULL,
    ...
    CONSTRAINT [UQ_BaseAddress] UNIQUE NONCLUSTERED ([BaseAddress] ASC)
);
